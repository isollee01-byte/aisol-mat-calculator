...
def simple_mode_calc_with_size(pyeong, area_type, expand_type, mat_side_cm):
    factor_800 = {"거실": 0.93, "거실+복도": 1.46,
                  "거실+복도+아이방1": 1.67, "거실+복도+주방": 2}
...
def login_screen():
    show_logo_top()
    st.markdown("<h2 style='text-align:center;'>매트 견적프로그램</h2>", unsafe_allow_html=True)
...
def calculator():
    ...
    selected_address = st.text_input("주소")
    detail_address = st.text_input("상세 주소")
    install_date = st.date_input("시공 희망일")

    st.subheader("📌 매트 재질 선택")
    material_type = st.selectbox("원단 재질", ["일반 TPU", "프리미엄 TPU", "패브릭 TPU"])
    material_price_map = {"일반 TPU": 39000, "프리미엄 TPU": 42000, "패브릭 TPU": 50000}
    material_unit_price = material_price_map[material_type]

    st.subheader("📌 매트 크기 선택")
    mat_size_str = st.selectbox("매트 크기", ["600×600", "700×700", "800×800", "1000×1000", "1200×1200"])
    ...
    if mode == "간편측정":
        ...
        for zone in zones:
            ...
            w = col1.number_input(f"{zone} 가로(cm)", min_value=0.0)
            h = col2.number_input(f"{zone} 세로(cm)", min_value=0.0)
            if w > 0 and h > 0:
                total_area += w * h
    ...
    if total_mats > 0:
        ...
        <div style='background:white; padding:30px; border-radius:8px;
                    width:800px; margin:auto; border:1px solid #ddd; font-size:18px;'>  
            ...
            시공비: {work_cost:,} 원<br>
            <hr>
            <b>최종 견적(VAT 포함): {total_price:,} 원</b><br><br>
